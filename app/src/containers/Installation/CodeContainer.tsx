...
export default function CodeContainer({ codeString }: CodeProps) {
  ...
  const [sendWidgetInstallation, { loading: isSending }] = useMutation(SEND_WIDGET_INSTALLATION);
  ...
  return (
    ...
    <div className="p-4 border-b border-blue-100/60 flex-shrink-0 bg-gradient-to-r from-white to-blue-50/50">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <h3 className="text-base font-bold text-gray-900 tracking-tight">Customize Your Widget</h3>
          <p className="text-sm text-gray-600 font-medium">Choose position and language</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
         ...
         <div className="space-y-2">
           <label className="block text-sm font-semibold text-gray-800 tracking-wide">Position</label>
        ...
        <div className="relative space-y-2" ref={dropdownRef}>
          <label className="block text-sm font-semibold text-gray-800 tracking-wide">Language</label>
          <button
            type="button"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-900 font-semibold text-sm truncate">{selectedLanguage.name}</span>
            </div>
            <FaChevronDown
              className={`w-4 h-4 text-gray-500 transition-all duration-300 ${isLanguageDropdownOpen ? "rotate-180 text-blue-500" : ""}`}
            />
          </button>
                ...
                {filteredLanguages.length > 0 ? (
                  filteredLanguages.map((lang) => (
                    ...
                    >
                      ...
                      {language === lang.code && <FaCheck className="w-4 h-4 text-blue-500 drop-shadow-sm" />}
                    </button>
                  ))
                ) : (
                  <div className="px-4 py-3 text-gray-500 text-sm font-medium">No languages found</div>
                )}
              </div>
            </div>
          )}
    ...
    <div className="flex-1 min-h-0 flex flex-col">
      <div className="p-4 bg-gradient-to-r from-white to-blue-50/30 flex-shrink-0">
        ...
        <p className="text-sm text-gray-600 font-medium">Paste before closing {"</body>"} tag</p>
      </div>

                           <div className={`bg-gradient-to-br from-gray-50 to-blue-50/30 p-3 border-y border-blue-100/60 flex-1 min-h-[100px] ${isExpanded ? 'max-h-none' : 'max-h-[100px]'} flex flex-col`}>
          <div className="bg-gray-900 rounded-xl border border-gray-700 p-4 hover:border-gray-600 hover:shadow-lg transition-all duration-300 flex-1 min-h-[80px] ${isExpanded ? 'max-h-none' : 'max-h-[80px]'} flex flex-col shadow-sm ring-1 ring-gray-800/50">
            {/* Code content */}
                                               <div className={`flex-1 min-h-[60px] ${isExpanded ? 'max-h-none' : 'max-h-[60px]'} bg-gray-800 rounded-lg p-3 w-full overflow-hidden relative`}>
                               <pre className={`text-base text-gray-100 font-mono leading-relaxed w-full p-0 m-0 ${isExpanded ? 'whitespace-pre-wrap' : 'overflow-x-hidden'}`}>
                  <code className={`block ${isExpanded ? 'whitespace-pre-wrap' : 'whitespace-nowrap'} text-gray-100`}>{formattedCodeString}</code>
                </pre>
             </div>
          </div>
        </div>
            ...
            <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-6 text-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold">Send Installation Instructions</h3>
                    <p className="text-purple-100 text-sm">We'll email you the setup code</p>
                  </div>
                </div>
                <button
              ...
              {!sendSuccess ? (
                <>
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all ${
                        emailError ? 'border-red-300 focus:border-red-500' : 'border-gray-300 focus:border-purple-500'
                      }`}
                      disabled={isSending}
                    />
                    {emailError && (
                  ...
                  >
                    {isSending ? (
                      <>
                         <CircularProgress size={24} color={'inherit'} className="w-4 h-4" />
                        Sending...
                      </>
                    ) : (
                      <>
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-bold text-gray-800 mb-2">Email Sent Successfully!</h3>
                  <p className="text-gray-600">Check your inbox for installation instructions.</p>
                </div>
              )}
            </div>
          </div>
