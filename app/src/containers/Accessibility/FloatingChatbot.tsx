...
const styles = {
  ...
  chatbotButton:
    'bg-blue-600 hover:bg-blue-700 transition-all duration-200 text-white rounded-full p-4 w-[68px] h-[68px] flex items-center justify-center cursor-pointer shadow-xl hover:shadow-2xl hover:scale-105',
  chatbotWindow: `
    fixed
    bottom-20
    right-6
  ...
  `,
  chatHeader:
    'bg-gradient-to-r from-blue-600 to-blue-700 text-white p-3 sm:p-4 flex justify-between items-center shadow-md min-h-[60px]',
  headerTitle:
    'text-sm sm:text-lg font-semibold flex items-center gap-1 flex-1 min-w-0',
  headerClose:
    'p-1.5 sm:p-2 hover:bg-blue-850 rounded-full transition-colors cursor-pointer flex-shrink-0',
  ...
  inputField:
    'flex-1 px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm transition-all',
  sendButton:
    'bg-blue-600 hover:bg-blue-700 transition-all duration-0 text-white rounded-xl w-12 h-12 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-md',
  sendIcon: 'w-5 h-5',
};
      ...
      const transformedData = scanResults
        ? {
            ...
            userProgress: userProgress || {
              ...
              preferredPlatform:
                scanResults.techStack?.accessibilityContext?.platform ||
                'Unknown',
            },
            fullScanData: {
              url: scanResults.url,
              ...
              widgetInfo: {
                result:
                  scanResults.scriptCheckResult === 'Web Ability'
                    ? 'WebAbility'
                    : 'None detected',
              },
              ...
              enhancedScore:
                scanResults.scriptCheckResult === 'Web Ability'
                  ? Math.min((scanResults.score || 0) + 45, 95)
                  : scanResults.score || 0,
              hasWebAbilityWidget:
                scanResults.scriptCheckResult === 'Web Ability',
              webAbilityBonus:
                scanResults.scriptCheckResult === 'Web Ability' ? 45 : 0,
              // Use your GraphQL structure for accessibility issues
              accessibilityIssues:
                scanResults.ByFunctions ||
                scanResults.axe?.errors ||
                scanResults.htmlcs?.errors ||
                [],
              ...
              issueCounts: {
                total:
                  scanResults.ByFunctions?.reduce(
                    (count: number, func: any) =>
                      count + (func.Errors?.length || 0),
                    0,
                  ) || 0,
                critical: 0,
                serious: 0,
              ...
              platform:
                ...
                'Unknown',
              platformType:
                scanResults.techStack?.accessibilityContext?.platform_type ||
                '',
  ...
  const generateWelcomeMessage = () => {
    if (scanResults?.techStack?.accessibilityContext?.platform) {
      const { platform, platform_type } =
        scanResults.techStack.accessibilityContext;
...
**What's your website URL?** 👇`;
  ...
  const submitIssueForAnalysis = useCallback(
    (issueDetails: string) => {
      setIsOpen(true);
  ...
  return (
    <div className={styles.chatbotContainer}>
      <button
        ...
        aria-label={
          isOpen
            ? 'Close AI Accessibility Assistant'
            : 'Open AI Accessibility Assistant'
        }
      >
        {isOpen ? (
          <svg
        ...
        <div className={styles.chatbotWindow}>
          <div className={styles.chatHeader}>
            <h3 className={styles.headerTitle}>
              <svg
                className="w-4 h-4 sm:w-5 sm:h-5 text-blue-100 flex-shrink-0"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <span className="truncate">Accessibility Helper</span>
              {getCurrentDomain() && (
                <span className="hidden sm:inline text-xs bg-blue-500 px-2 py-1 rounded-full">
                  {getCurrentDomain()}
                </span>
              )}
            </h3>
            <div className="flex items-center gap-1 flex-shrink-0">
              <button
                ...
                className="p-1.5 sm:p-2 hover:bg-blue-850 rounded-full transition-colors cursor-pointer flex-shrink-0"
                ...
                <div className="markdown-content prose prose-sm max-w-none">
                  <ReactMarkdown
                    components={{
                      ...
                      code: ({ children }: any) => (
                        <code className="bg-gray-100 px-1 py-0.5 rounded text-sm font-mono overflow-x-auto block">
                          {children}
                        </code>
                      ),
                      pre: ({ children }: any) => (
