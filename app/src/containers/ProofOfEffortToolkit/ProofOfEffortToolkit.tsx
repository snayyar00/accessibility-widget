...
const ProofOfEffortToolkit: React.FC = () => {
  ...
  const handleEmailSubmit = async (email: string) => {
      ...
      //  console.log('Step 3: Fetching report data by R2 key...');
        try {
          const { data } = await fetchReportByR2Key({
            variables: { r2Key: latestReport.r2_key }
          });
          reportDataForPdfs = data?.fetchReportByR2Key;
        //  console.log('Report data fetched:', reportDataForPdfs ? 'success' : 'null');
        } catch (error) {
          console.error('Error fetching report data:', error);
          // Continue without report data - we'll still generate the intro PDF
        }
      } else {
        console.warn('No latest report or valid r2_key found for PDF generation. Monthly audit report and accessibility statement PDFs will be skipped.');
      }
    ...
    //  console.log('Step 4: Generating PDFs...');
      ...
      // Generate monthly audit report PDF if we have report data
      if (reportDataForPdfs) {
        try {
      ...
      // Generate accessibility statement PDF if we have report data
      if (reportDataForPdfs) {
        try {
    ...
    try {
      ...
      try {
        ...
        if (latestReport && latestReport.r2_key) {
          ...
          // If no report key available, create a placeholder PDF
          const { jsPDF } = await import('jspdf');
          const placeholderDoc = new jsPDF();
          placeholderDoc.setFontSize(18);
          placeholderDoc.setTextColor(220, 38, 38);
