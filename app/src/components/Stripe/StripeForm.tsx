  ...
  const validateCouponCode = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/validate-coupon`,
        {
          method: 'POST',
          headers: {
    ...
    <form onSubmit={onSubmit} className={`w-[300px] ${className}`}>
      <div>
        {onGoBack && (
          <Button
            type="button"
            onClick={onGoBack}
            className="border-transparent p-0 mb-5 cursor-pointer [&>svg]:text-red [&>svg]:w-5"
          />
        )}
        <div>
      ...
      >
        {isSubmitting
          ...
          : submitText ?? t('Common.text.submit')}
      </Button>
    </form>
  );
};
