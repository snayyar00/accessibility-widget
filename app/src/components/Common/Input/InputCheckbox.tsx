...
const Checkbox = React.forwardRef(
  (
    ...
    <span 
      className="relative w-6 h-6 inline-block rounded overflow-hidden border-[1px] border-solid"
      style={{ borderColor: '#6C7586' }}
    >
      {" "}
      <input
        type={type}
        ...
        className={`absolute opacity-0 cursor-pointer w-6 h-6 [&:checked~.checkmark]:bg-semi-primary [&:checked~.checkmark::after]:block ${className}`}
        {...props}
        ref={ref}
        id={id}
      />{" "}
