 ...
 *
 * Security: This function does not perform access checks. Callers must ensure the requesting user is allowed
 * to access the given organization (e.g. via isAuthenticated + allowedOrganization or service-layer org access)
 * before calling, so SMTP credentials are not exposed to unauthorized callers.
 */
export async function getOrganizationSmtpConfig(organizationId: number): Promise<OrganizationEmailContext | null> {
  ...
  // Fallback: settings.smtp. Require explicit host and port.
