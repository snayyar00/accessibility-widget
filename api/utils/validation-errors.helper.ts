import { ApolloError } from './graphql-errors.helper'
...
export function createValidationError(code: ValidationErrorCode, message?: string): ApolloError {
  return new ApolloError(message || 'Validation failed', code)
}
...
export function createMultipleValidationErrors(errors: string[]): ApolloError {
  ...
  return new ApolloError('Multiple validation errors', ValidationErrorCode.GRAPHQL_VALIDATION_FAILED, {
...
export function getValidationErrorCode(errors: string[]): ValidationErrorCode {
  ...
  const errorCategories = {
    emailPlus: errors.some((error) => error.includes('Email addresses with + symbol are not allowed')),
    nameLinks: errors.some((error) => error.includes('Name contains invalid characters or links')),
    emailFormat: errors.some((error) => error.toLowerCase().includes('email') && (error.toLowerCase().includes('invalid') || error.toLowerCase().includes('required'))),
    passwordWeak: errors.some((error) => error.toLowerCase().includes('password') && (error.toLowerCase().includes('weak') || error.toLowerCase().includes('characters'))),
    nameRequired: errors.some((error) => error.toLowerCase().includes('name') && error.toLowerCase().includes('required')),
    urlInvalid: errors.some((error) => error.toLowerCase().includes('url') || error.toLowerCase().includes('domain')),
  }
