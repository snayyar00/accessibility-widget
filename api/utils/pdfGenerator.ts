...
export async function generateAccessibilityReportPDF(reportData: any, url: string, widgetStatus: string = 'false', language: string = 'en'): Promise<Buffer> {
  const doc = new jsPDF();
  ...
  const [translatedStatus, translatedMessage, translatedMild, translatedModerate, translatedSevere, translatedScore, translatedIssue, translatedIssueMessage, translatedContext, translatedFix, translatedLabel, translatedTotalErrors] = [
    status,
    message,
    'Mild',
    'Moderate',
  ...
  try {
    if (logoBase64) {
      ...
      doc.roundedRect(containerX, containerY, containerW, containerH, 4, 4, 'F');
  ...
  doc.roundedRect(containerX, containerY, containerWidth, containerHeight, 4, 4, 'FD');
    ...
    tableBody.push([
      {
        ...
        styles: {
          ...
          halign: 'left',
          cellPadding: 10,
          fillColor: issue.impact === 'critical' ? [255, 204, 204] : issue.impact === 'Mild' ? [225, 245, 254] : issue.impact === 'moderate' ? [187, 222, 251] : [248, 250, 252],
          font: 'courier',
          minCellHeight: 30,
        },
      },
