...
import { createMeetingRequest, createQuoteRequest, getUserMeetingRequests, getUserQuoteRequests, MeetingRequestData, QuoteRequestData } from '../../repository/serviceRequest.repository'
...
const serviceRequestResolvers = {
  Query: {
    getUserQuoteRequests: combineResolvers(isAuthenticated, async (_: unknown, __: unknown, { user }) => {
      try {
    ...
    getUserMeetingRequests: combineResolvers(isAuthenticated, async (_: unknown, __: unknown, { user }) => {
      try {
  ...
  Mutation: {
    createQuoteRequest: combineResolvers(isAuthenticated, async (_: unknown, { input }: CreateQuoteRequestInput, { user }) => {
      try {
    ...
    createMeetingRequest: combineResolvers(isAuthenticated, async (_: unknown, { input }: CreateMeetingRequestInput, { user }) => {
      try {
